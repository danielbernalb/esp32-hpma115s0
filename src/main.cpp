...
void sensorInit()
{
  ...
  if (response[0] == 0x16 && response[1] == 0x01 && response[2] == 0x10 && response[3] == 0xD9)
    Serial.println("done");
  else
...
int CO2CM1106val()
{
  ...
  {
    while (co2cm1106.available() > 0)
      char t = co2cm1106.read(); // Clear serial input buffer;
    return -1;
  }
}
#endif
...
void buttonloop()
{
  static byte cmd2[6] = {0x11, 0x03, 0x03, 0x01, 0x90, 0x58}; // Set to 400ppm
  //51
  static byte response2[4] = {0};
  ...
  if (digitalRead(BUTTON) == LOW)
  {
    ...
    if (digitalRead(BUTTON) == LOW)
    {
      ...
      if (digitalRead(BUTTON) == LOW)
      {
        ...
        co2cm1106.write(cmd2, 10);
        co2cm1106.readBytes(response2, 4);
        Serial.println("Resetting forced calibration factor to : 400");
        if (response2[0] == 0x16 && response2[1] == 0x01 && response2[2] == 0x03 && response2[3] == 0xE6)
          Serial.println("done");
        else
          Serial.println(" failed");
        delay(5000);
...
#ifdef CM1106
  gui.welcomeAddMessage("PREHEAT timer 30 seconds:");
  Serial.print("Preheat timer 30 seconds:");
  for (resetvar = 0; resetvar <= 31; resetvar++)
  {
    delay(1000);
    gui.welcomeAddMessage(String(resetvar));
    Serial.println(resetvar);
